...
document.addEventListener('DOMContentLoaded', function() {
    // Seleciona TODOS os submenu-toggles, independentemente do nível
    const submenuToggles = document.querySelectorAll('.sidebar .submenu-toggle');
    ...
    submenuToggles.forEach(toggle => {
        toggle.addEventListener('click', function(event) {
            ...
            // Encontra o submenu IMEDIATAMENTE após este toggle
            const submenu = this.nextElementSibling; 

            if (submenu && submenu.classList.contains('submenu')) {
                // Alterna a classe 'open' no toggle clicado
                this.classList.toggle('open');
    ...
    // Lógica para marcar o link ativo e abrir os submenus pais
    const currentLocation = window.location.pathname;
    const allLinks = document.querySelectorAll('.sidebar .menu-principal a');
    let bestMatchLink = null;
    let longestMatchLength = 0;
    ...
    if (bestMatchLink) {
        ...
        // Abre todos os submenus pais do link ativo
        let currentElement = bestMatchLink;
        while (currentElement && currentElement.classList && !currentElement.classList.contains('menu-principal')) {
            if (currentElement.classList.contains('submenu')) {
                currentElement.style.display = 'block'; // Garante que o submenu está visível
